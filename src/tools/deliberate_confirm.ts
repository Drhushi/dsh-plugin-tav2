...
export interface DeliberateConfirmResult {
  ok: boolean
  action: string
  affected: number
  text: string
...
/**
 * app 层纯函数：推敲定论（list 只读；approve/reject/update 是写操作，需 approve 放行）。
 * CLI 与 dsh 共用（单一事实源）；approve 注入审批决策（dsh 走 ctx.approval，CLI 走 --yes 映射）。
 */
export async function runTsDeliberateConfirm(
  config: Config,
  args: DeliberateConfirmArgs,
  deps: { approve: (reason: string) => Promise<ApprovalDecision> },
): Promise<DeliberateConfirmResult> {
...
export function registerDeliberateConfirmTool(ctx: Context, config: Config): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args: DeliberateConfirmArgs, exec): Promise<DeliberateConfirmResult> {
      // 薄壳：审批桥注入 app 层纯函数（CLI 若需要可复用 runTsDeliberateConfirm）。
      const approve = (reason: string) => requestApproval(ctx, exec, reason)
      return runTsDeliberateConfirm(config, args, { approve })
    },
  }))
}
