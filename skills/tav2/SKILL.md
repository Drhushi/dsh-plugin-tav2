---
name: tav2
description: 游戏本地化标准工作流（引擎无关；当前完整适配 Ren'Py）——初始化、术语扫描/推敲/锁定、世界书提名、分批翻译、审校回填、完整性校验、补丁打包/迁移/部署/卸载。工具为独立 CLI `tav2kit`（零 dsh 依赖，--json 机器可读，退出码 0 成功 / 1 业务失败 / 2 用法错误 / 4 写操作需 --yes 确认）。
when_to_use: 用户要做游戏本地化/汉化/翻译（Ren'Py 完整支持；Unity/Yarn 布局识别即拒绝提示暂未适配）、续接翻译进度、术语管理、世界书、审校、打包发布补丁时
---
